 ...
 * do podmiany na docelowe zdjęcie floty SPRINT-TRANS bez zmiany layoutu (patrz
 * komentarze przy `heroImage`/ScrollBus).
 */
export function HeroSection() {
  return (
    <section className="relative overflow-hidden bg-brand-navy pt-24 pb-48 text-white sm:pt-28 sm:pb-56 lg:pt-32 lg:pb-64">
      {/* Placeholder tła — głęboka, warstwowa poświata budująca fotograficzny,
          premium charakter (zamiast płaskiego koloru). Docelowe zdjęcie trasy/
          autobusu wstawiamy jako <img> absolutnie pozycjonowany NAD tą warstwą
          (z-0) i POD warstwą kontrastu poniżej (z-[1]) — dzięki temu tekst po
          lewej zawsze pozostanie czytelny niezależnie od jasności zdjęcia. */}
      <div
        aria-hidden="true"
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl gap-14 px-6 lg:grid-cols-[1.05fr_1fr] lg:items-center lg:gap-8 lg:px-8">
        ...
        <ScrollReveal delay={300} direction="right" className="relative">
          {/* Miejsce docelowe pod zdjęcie autokaru SPRINT-TRANS — wyraźnie
              wydzielona "scena" (poświata + platforma pod kołami) z placeholderem
              SVG reagującym na scroll, gotowa na podmianę na finalną fotografię. */}
          <div
            aria-hidden="true"
            className="absolute inset-0 scale-125 rounded-full opacity-80"
            style={{
              background: "radial-gradient(closest-side, color-mix(in oklch, var(--color-primary), transparent 55%), transparent)",
            }}
          />
          <div className="relative mx-auto aspect-4/3 w-full max-w-2xl lg:max-w-none">
            ...
            <div
              aria-hidden="true"
              className="absolute inset-x-6 bottom-6 h-4 rounded-full bg-black/40 blur-xl sm:inset-x-10"
            />
          </div>
        </ScrollReveal>
      </div>
